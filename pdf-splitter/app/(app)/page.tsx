  ...
  return (
    <div className="space-y-6">
      {!ready ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
          ...
          <div className="mt-1 text-sm">
            Go to <Link className="font-semibold underline" href="/references">References</Link> and upload Saad + Gorman Excel files.
          </div>
        </div>
      ) : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Link
          href="/references"
          className="rounded-xl border bg-white p-6 shadow-sm hover:bg-zinc-50"
        >
              ...
              {jobs.length === 0 ? (
                <tr className="border-t">
                  <td className="px-4 py-4 text-zinc-600" colSpan={6}>
                    No jobs yet. Go to <Link className="font-semibold text-teal-700 hover:text-teal-800" href="/process">Process</Link>.
                  </td>
                </tr>
              ) : (
                jobs.map((job) => {
                    ...
                    <tr key={job.id} className="border-t">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2">
                          <a className="text-xs font-semibold text-zinc-900 underline hover:text-zinc-700" href={`/api/download-pdf/${job.id}/saad`}>
                            Saad
                          </a>
                          <a className="text-xs font-semibold text-zinc-900 underline hover:text-zinc-700" href={`/api/download-pdf/${job.id}/gorman`}>
                            Gorman
                          </a>
                          <a className="text-xs font-semibold text-zinc-900 underline hover:text-zinc-700" href={`/api/download-pdf/${job.id}/extra`}>
                            Extra
                          </a>
                        </div>
                      </td>
